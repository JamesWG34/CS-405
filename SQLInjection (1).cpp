...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    {
        // Get conditions after WHERE
        std::string condition_part = lower_sql.substr(where_pos);

        // Match: OR value = value (quoted or not)
        std::regex injection_pattern(R"(\sor\s+['"]?\w+['"]?\s*=\s*['"]?\w+['"]?)");

        // If found, it's an injection attempt
        if (std::regex_search(condition_part, injection_pattern))
        {
            std::cout << "[!] SQL Injection attempt detected. Query blocked:\n" << sql << std::endl;
            return false;
        }
    }
